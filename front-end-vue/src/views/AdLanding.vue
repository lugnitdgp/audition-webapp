							...
							<v-card class="mx-auto"  outlined color="#2196F3">
								<v-list-item three-line>
									<v-list-item-content>
										...
										<v-list-item-title class="display-2">{{items.length}}</v-list-item-title>
									</v-list-item-content>
				...
				>
					<template v-slot:item="row">
						<tr>
							...
							<td>
								<v-btn class="mx-2" light small @click="usercontrol(row.item._id)"
									>Detail</v-btn
								>
							</td>
						</tr>
...
export default {
	...
	beforeCreate() {
		if (localStorage.getItem("token") === null) {
			...
			axios
				.get("http://localhost:3000/auth/jwt", {
				...
				.then(res => {
					if (res.data.isAdmin === true) {
						...
						alert(
							"You are trying to access higher level access and have been flagged in our audition process"
						);
